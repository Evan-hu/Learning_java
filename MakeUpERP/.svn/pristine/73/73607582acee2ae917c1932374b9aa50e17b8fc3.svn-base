...
if(alertMsg){
  if (xhr.status == 417) {
     var rtnJson = eval("("+xhr.responseText+")");     
  	alertMsg.error("<div>"+rtnJson.message+"</div>");
  } else { 
...
if(json.statusCode==DWZ.statusCode.error){
   ...
   if ("closeDialog" == json.callbackType) {
...
hoverClass:function(className){
  ...
  return this.each(function(){
    ...
    function(){       
       ...
       if ($this.isTag("tr") && $this.attr("in")) {
       	  showFloatDiv(this);	  		
	   }
    },
     function(){
       $(this).removeClass(_className);
       if ($this.isTag("tr") && $this.attr("out")) {
          hideFloatDiv(this);
       }
     }
    );   
...
/******闁插秶鐤嗘径宥夛拷閺嶏拷******/
checkTreeValueArray = new Array();
selectTreeRankNum = 0;
/*************/
var op=$.extend({checkFn:null,selected:"selected",exp:"expandable_tree",coll:"collapsable_tree",firstExp:"first_expandable",firstColl:"first_collapsable",lastExp:"last_expandable",lastColl:"last_collapsable",folderExp:"folder_expandable",folderColl:"folder_collapsable",endExp:"end_expandable",endColl:"end_collapsable",file:"file",ck:"checked",unck:"unchecked"},options);
return this.each(function(){
var $this=$(this);
...
showlv:($this.attr("showlv")?$this.attr("showlv"):0),
...
var sTarget = $(this).attr("param");
var $grid = $this.parent(); // tree
if (sTarget) {
    if ($("#row_data", $grid).size() == 0) {
 	  $grid.prepend('<input id="row_data" type="hidden" />');
	}
	$("#row_data", $grid).val(sTarget);	
}
...
var op=$.extend({icon:options.icon,ckbox:options.ckbox,exp:"",coll:"",showSub:false,level:0,options:null,isLast:false,showlv:1},options);
return this.each(function(){
var node=$(this);
var tree=$(">ul",node);
...
if ($this.attr("multi") && $this.attr("multi") == "false" ) {
  options.multi = false;
} else {
  options.multi = true;
}
...
if($this.attr("onOpen")){
options.onOpen=DWZ.jsonEval($this.attr("onOpen"));}
...
var op=$.extend({data:{},fresh:true,external:false},options);
...
open:function(url,dlgid,title,options) {
 $.pdialog._open(url,{},dlgid,title,options);
},
_open:function(url,data,dlgid,title,options){
...
$tr.click(function(){
 ...
 var sRowSelect = $tr.attr("rowselect");
 if (sRowSelect) {
   $('#'+sRowSelect).trigger("click");
 }
}
);});
...
if(form){
   var butt = $("input[type=button]",form);
   if (butt[0]){
	 butt.trigger("click");
   } else {
     $box.ajaxUrl({type:"POST",url:$(form).attr("action"),data:$(form).serializeArray(),callback:function(){$box.find("[layoutH]").layoutH();}});
   }
 }
} else{
...
$box.find(":input").each(function(){
  var $input=$(this),inputName=$input.attr("suggestname");
  if (!inputName) {
...
$.pdialog.close("selewin");
$.bringBackSuggest(args);
//$.pdialog.closeCurrent();
},
bringBackSuggestPop: function(args){
	var $box = $("#"+args.target.replace(".","\\.").replace("[","\\[").replace("]","\\]")).parents(".unitBox:first");
	$box.find(":input").each(function(){
	  var $input = $(this), inputName = $input.attr("suggestname");
	  var suffix = args.target.replace("Link","");		
	  if (!inputName) {
     	inputName=$input.attr("name");
  	  }		  	  
	  for (var key in args) {
...
clearBackSuggestPop: function(target){
 	$.pdialog.close("selewin");
	var $box = $("#"+target.replace(".","\\.").replace("[","\\[").replace("]","\\]")).parents(".unitBox:first");
	$box.find(":input").each(function(){
	  var $input = $(this), inputName = $input.attr("lookupgroup");
	  var suffix = target.replace("Link","");		
	  if (suffix == inputName) {
     	$input.val("");  	
  	  }		  	    
    });
},
bringBackPop: function(args){
 $.pdialog.close("selewin");
 $.bringBackSuggestPop(args);
}
});
$.fn.extend({
lookup:function(){
...
alertMsg.error($this.attr("warn")||DWZ.msg("alertSelectMsg"));
return false;}
$.pdialog.open(url,"selewin",$this.attr("title")||$this.text(),options);
return false;});});},
multLookup:function(){
return this.each(function(){
var $this=$(this),args={};
...
for (var i=0; i<suggestFields.length; i++){
  if (suggestFields[i].indexOf("callback_") ==0) {
    callback = suggestFields[i].substring(9);
    break; 
  }
}
var dispFields;
if ($input.attr('dispFields')) {
  dispFields =$input.attr('dispFields').split(",");
} else {
  dispFields =suggestFields;
}
...
for(var i=0;i<suggestFields.length;i++){
  ...
  if(str){
    if(liLabel)liLabel+='-';
    for (var t=0;t<dispFields.length;t++) {
      if (dispFields[t] == suggestFields[i]) {
     	liLabel+=str;
     	break;   
      }
    }    
...
if(addButTxt){
var $addBut=$('<div style="display:none;" class="button"><div class="buttonContent"><button id="addRowBtn" type="button">'+addButTxt+'</button></div></div>').insertBefore($table).find("button");
var $rowNum=$('<input style="display:none;" type="text" name="dwz_rowNum" class="textInput" style="margin:2px;" value="1" size="2"/>').insertBefore($table);
var trTm="";
...
$(':input,img,a.btnLook',this).each(function(){
...
if ($this.attr('href')) {$this.attr('href', $this.attr('href').replaceSuffix(i));}
...
var readonly = "";
if (field.fieldClass == "readonly") {
   readonly = "readonly=readonly";
}
switch(field.type){
	case 'del':
	html='<a href="javascript:void(0)" class="btnDel '+field.fieldClass+'">鍒犻敓鏂ゆ嫹</a>';
	break;
	case 'lookup':
	var suggestFrag='';
	if(field.suggestFields) {
	  ...
	  //'<input type="hidden" name="'+field.lookupGroup+'.'+field.lookupPk+suffix+'"/>'+
	  html='<input readonly="readonly"  type="text" name="'+field.name+'"'+suggestFrag+' lookupPk="'+field.lookupPk+'" style="'+field.size+'" class="'+field.fieldClass+'"/>'
	      +'<a class="btnLook"  '+readonly+' id="'+field.lookupGroup+'Link"  href="'+field.lookupUrl+'&poptarget='+field.lookupGroup+'Link" lookupGroup="'+field.lookupGroup+'" '+suggestFrag+' lookupPk="'+field.lookupPk+'" title="閿熸枻鎷烽敓鎻揪鎷烽敓锟介敓锟�>閿熸枻鎷烽敓鎻揪鎷烽敓锟介敓锟�a>';
	   //html='<input readonly="readonly"  type="text" name="'+field.name+'"'+suggestFrag+' lookupPk="'+field.lookupPk+'" style="'+field.size+'" class="'+field.fieldClass+'"/>'
	   //  +'<a class="btnLook" id="'+field.lookupGroup+'Link"  '+readonly+'  href="'+field.lookupUrl+'&poptarget='+field.lookupGroup+'Link" lookupGroup="'+field.lookupGroup+'" '+suggestFrag+' lookupPk="'+field.lookupPk+'" title="閫夐敓鏂ゆ嫹">閫夐敓鏂ゆ嫹</a>';	   
	break;
	case 'attach':
	//html='<input type="hidden" name="'+field.lookupGroup+'.'+field.lookupPk+suffix+'"/>'+'<input type="text" name="'+field.name+'" '+field.size+' readonly="readonly" class="'+field.fieldClass+'"/>'+'<a class="btnAttach" href="'+field.lookupUrl+'" lookupGroup="'+field.lookupGroup+'" '+suggestFrag+' lookupPk="'+field.lookupPk+'" width="560" height="300" title="閿熸枻鎷烽敓鎻揪鎷烽敓锟介敓锟�>閿熸枻鎷烽敓鎻揪鎷烽敓锟介敓锟�a>';
	html='<img name="'+field.name+'" '+field.lookupPk + ' ></img>';
	break;
	case 'enum':
	$.ajax({
	...
	break;
	case 'date':
	html='<input '+readonly+' type="text" name="'+field.name+'"  value="'+field.defaultVal+'" class="date '+field.fieldClass+'" format="'+field.patternDate+'" style="'+field.size+'"/>'+'<a class="inputDateButton" href="javascript:void(0)">閫夐敓鏂ゆ嫹</a>';
	break;
		case 'select':
	    var vList = field.postField.split(",");
	    var dList = field.suggestFields.split(",");
	    html = '<select id="'+field.name+'" size="1" name="'+field.name+'">';
	    html = html + '<option value=""></option>';
	    var leng = vList.length;
	    var selected = "";
	    for (var i=0;i<leng;i++) {	      
	      selected = "";	      	
	      if (field.defaultVal == vList[i]) {
	         selected = "selected";
	      }     
	      html = html + '<option '+selected+' value="'+vList[i]+'">'+dList[i]+'</option>';
	    }
	    html = html + '</select>';
	    break;
	default:
	html='<input  '+readonly+'  type="text" name="'+field.name+'" value="'+field.defaultVal+'" style="'+field.size+'" class="'+field.fieldClass+'"/>';
	break;
}
if (field.size == 'display:none') {
