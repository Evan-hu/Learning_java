...
<html>
  ...
  <link rel="stylesheet" href="<%=basePath%>css/newscss/activity.css" />
  <link rel="stylesheet" href="<%=basePath%>css/newscss/activity_effect.css" />
  </head>
  ...
  <div class="mh">
                  <c:if test="${code == 'news' }">
                      当前位置 : <a href="<%=basePath%>article/list.do?code=news&&tab=1">非泊新闻</a>  > 新闻详情
                  </c:if> 
                  <c:if test="${code == 'active' }">
                      当前位置 : <a href="<%=basePath%>article/list.do?code=active&&tab=1">最新活动</a>  > 活动详情
                  </c:if> 
                    </div>
                    </div>
        <div class="container">
                      ...
                      <div class="navPanel">
                        ...
                        <div class="mcd">
                                  <DIV class="MainBg">
                                        ...
                                        <div class="subtitle">
                                          <span class="pubdate">发布时间 : <fmt:formatDate pattern="yyyy-MM-dd" value="${article.createTime}" type="both"/></span>
                                            <span class="sub_share">
                                              <span class="sub_share_l">新闻分享</span>
                                              <div class="jiathis_style" style="float:left; margin-top:11px;">
                                        ...
                                        <c:if test="${front != 0}">
                                           <p>上一篇: <a href="<%=basePath%>article/detailList.do?articleId=${front}&&code=${code}">${fronttitle}</a></p>
                                        </c:if>
                                        <c:if test="${behind != 0}">
                                           <p>下一篇: <a href="<%=basePath%>article/detailList.do?articleId=${behind}&&code=${code}">${behindtitle}</a></p>
                                        </c:if>
                                        </div>  
                                  </DIV> 
                             </div>
