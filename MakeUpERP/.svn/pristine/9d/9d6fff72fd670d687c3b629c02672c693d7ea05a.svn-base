...
public class AdjustCommBiz extends ACLBiz{

	public AdjustCommBiz(UserACL userACL) {
		super(userACL);
	}

	public PageResult queryStockList(QueryPageData queryPageData,List<DbField> dbFieldList) {
		String sql = "SELECT idc.*, s.Name ,ia.BILLS_NUM FROM INNAGE_ADJUST_COMM idc " +
				"JOIN COMMODITY c ON c.`COMMODITY_ID`=idc.`COMMODITY_ID` " +
				"JOIN STOCK s ON idc.`STOCKS_ID`=s.STOCK_ID JOIN INNAGE_ADJUST ia ON ia.`INNAGE_ADJUST_ID`=idc.`INNAGE_ADJUST_ID`;";
		return BizUtil.queryListBySQL(sql, "", "i.INNAGE_ADJUST_COMM_ID desc",
