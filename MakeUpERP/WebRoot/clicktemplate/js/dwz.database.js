 ...
 */
(function($){
	var _lookup = {currentGroup:"", suffix:"", $target:null,$targetold, pk:"id"};
	var _util = {
	...
	$.fn.extend({
		...
		suggest: function(){
			...
			return this.each(function(){
				...
				var dispFields=$input.attr('dispFields').split(",");
			...
			function initSuffix($tbody) {
				$tbody.find('>tr').each(function(i){
					$(':input, a.btnLook', this).each(function(){
						...
						if ($this.attr('id')) {$this.attr('id', lookupGroup.replaceSuffix(i));}
				...
				switch(field.type){
					...
					case 'lookup':
						...
						html = '<input type="hidden" name="'+field.lookupGroup+'.'+field.lookupPk+suffix+'"/>'
							...
							+ '<a class="'+field.lookupGroup+'Link" href="'+field.lookupUrl+'&poptarget='+field.lookupGroup+'Link" lookupGroup="'+field.lookupGroup+'" '+suggestFrag+' lookupPk="'+field.lookupPk+'" title="查找带回">查找带回</a>';
						break;
					case 'attach':
						html = '<input type="hidden" name="'+field.lookupGroup+'.'+field.lookupPk+suffix+'"/>'
					...
					case 'date':
						...
						break;
					case 'select':
					    var vList = postField.split(",");
					    var dList = suggestFields.split(",");
					    html = '<select id="'+field.name+'" size="1" name="'+field.name+'">';
					    html = html + '<option value=""></option>';
					    for (var i=0;i<vList.length;i++) {					      
					      html = html + '<option value="'+vList[i]+'">'+dList[i]+'</option>';
					    }
					    html = html + '</select>';
					    break;
					default:
