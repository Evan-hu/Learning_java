...
function showTip(divFormError,caller,promptText,type,ajaxed){
	...
	/* POSITIONNING */
	if($.validationEngine.settings.promptPosition == "topRight"){callerleftPosition +=  callerWidth -130; callerTopPosition += -inputHeight ; }
...
(function($) {
	...
	if($.validationEngineLanguage){				// IS THERE A LANGUAGE LOCALISATION ?
 	...
 	settings = jQuery.extend({
		...
		showArray:false, //bluespring修改 添加是否显示提示箭头，默认不显示
...
$.validationEngine = {
	...
	buildPrompt : function(caller,promptText,type,ajaxed) {			// ERROR PROMPT CREATION AND DISPLAY WHEN AN ERROR OCCUR
		...
		if($(deleteItself)[0]){
			$(deleteItself).get(0).validateField.unbind('mouseover',showTip).unbind('mouseout',$.validationEngine.hideTip);
			$(deleteItself).stop();
			$(deleteItself).remove();
		}
		...
		if($.validationEngine.settings.showOnMouseOver){
			...
			$(divFormError).get(0).validateField.bind('mouseover',$(divFormError),function(){showTip($(divFormError),caller,promptText,type,ajaxed)}).bind('mouseout',$(divFormError),$.validationEngine.hideTip);	
		}else{
